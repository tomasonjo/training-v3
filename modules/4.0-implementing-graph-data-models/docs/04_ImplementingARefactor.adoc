...
:slug: 04-igdm-40-implementing-refactor
:doctype: book
:toc: left
:toclevels: 4
:imagesdir: ../images
...
[source,cypher]
----
CREATE CONSTRAINT flightConsraint ON (f:Flight) ASSERT f.flightId IS UNIQUE
----
...
The query changes to work with the new model:
...
773 db hits here which is OK. Adding an index to Flight on date does not improve the query.
...
[.notes]
--
//Elaine - fix up???? numbers don't make sense
Here we see that the query has 61 db hits, much lower than what we had before so the refactor helped this query.
[source,cypher]
----
PROFILE
MATCH  (origin:Airport)-[connection:CONNECTED_TO]->(destination:Airport)
...
[.statement]
What Cypher clause do you use analyze the performance of a query?
...
[.statement]
When a query executes, the execution plan uses what data structure passes data from one step of the plan to the next?
