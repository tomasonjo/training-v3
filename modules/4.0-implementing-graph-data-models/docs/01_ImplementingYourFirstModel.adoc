...
For large graphs, the node properties used for the MERGE should have unique indexes on them.

[.statement]
In the next exercise, Exercise 1, you will:
[.small]
--
. Create three Airport nodes and two _CONNECTED_TO_ relationships per the initial data model by using the `MERGE` statement.
...
. Create a local 4.x database in the project and start it.
