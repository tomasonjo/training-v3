...
:prevsect: 2
:currsect: 3
:nextsecttitle: Neo4j Causal Clustering
:nextsect: 4
:experimental:
:imagesdir: ../images
...
Here is a brief overview of the default folders you will frequently use for managing the Neo4j instance in Debian.
...
=== Post-install: Changing the _neo4j_ password
...
You will learn about changing the _neo4j_ password on Debian later in this module.
...
When the instance is started, it creates a database folder named *neo4j* in the default location which is a folder under */var/lib/neo4j/data/databases*. You can start and stop the instance regardless of whether the _neo4j_ service is enabled.
...
You can view the folder created for the database:
...
----
sudo /usr/bin/neo4j-admin copy --from-database=<sourceDB> --to-database=<targetDB>
----
[NOTE] If you have neo4j credentials for your system, you need not use `sudo` here or need to change ownership to `neo4j:neo4j below`.
[start=5]
. Change the ownership to neo4j:neo4j recursively for the sub-folder for targetDB under the databases and transactions folders, for example:
----
sudo chown -R neo4j:neo4j /var/lib/neo4j/data/databases/<targetDB>
sudo chown -R neo4j:neo4j /var/lib/neo4j/data/transactions/<targetDB>
----
[start=6]
. Restart the Neo4j instance. (Note that if you did not have to change ownership, there is no need to restart Neo4j.)
...
----
sudo /usr/bin/neo4j-admin dump --database=<sourceDB> --to=<absolute path to dump file>
----
...
. Use the `load` command of the `neo4j-admin` tool to create the database from the dump file as follows:
----
sudo /usr/bin/neo4j-admin load --database=<targetDB> --from=<absolute path to dump file>
----
[NOTE] If you have neo4j credentials for your system, you need not use `sudo` here or need to change ownership to `neo4j:neo4j` below.
[start=5]
. Change the ownership to neo4j:neo4j recursively for the sub-folder for targetDB under the databases and transactions folders, for example:
...
If you do not want the databases used by the Neo4j instance to reside in the same location as the Neo4j installation, you can modify its location in the *neo4j.conf* file. If you specify a new location for the data, it must exist in the filesystem and the folder must be owned by _neo4j:neo4j_.
...
Just as you needed to change the password for the admin user, neo4j the first time you accessed a database, you must do the same here:

image::passwordChangeNewLocation.png[passwordChangeNewLocation,width=800,align=center]

[.title.has-gold-background.has-team-background]

// Elaine continue here
