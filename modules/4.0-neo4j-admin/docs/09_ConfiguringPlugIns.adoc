...
----
echo "CALL dbms.procedures() YIELD name;" | /usr/bin/cypher-shell -u neo4j -p <InstancePassword> --format plain > /usr/local/work/Procedures.txt
----
...
Here is an example of a generated *Procedures.txt* file:
