...
In this training, all screenshots and examples are shown using Neo4j Enterprise Edition installed as a Debian package. Some are from the Neo4j instance running on Debian and some are from the Docker instance, similar to what you will be working with for the hands-on exercises.
...
image::Neo4j_started.png[Neo4j_started,width=1200,align=center]
...
image::JournalEnd.png[JournalEnd,align=center,width=900]
...
For a Docker Neo4j instance, you use the *logs* command.
...
For all hands-on exercises of this training, you will using the Docker Neo4j instance.

With Docker, there is no PID as there are on other systems for checking the status of the Neo4j instance.
That is, `systemctl status neo4j` cannot be used to check the status of the Neo4j instance.

Instead, you can view the log to check the status:
