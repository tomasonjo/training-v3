...
[start=3]
. Restart the Neo4j instance.
. Ensure that it starts with no errors.
...
. You should have created the dump file for the movie database in the *~/files directory*.
. Stop the Neo4j instance.

*Exercise steps*:

. Open a terminal on your system.
. Create a folder named *~/neo4j-databases*. This is the folder where the databases will reside which is different from the default location used by Neo4j.
. Make sure that this *neo4j-databases* folder is owned by _neo4j:neo4j_. For example, navigate to the *~/* folder and enter `sudo chown neo4j:neo4j neo4j-databases`.
. Modify the *neo4j.conf* file to use */home/ubuntu/neo4j-databases* as the data directory. Also ensure that there the default database is maindb. Your *neo4j.conf* file should look something like this:
...
. Examine the files in the */home/ubuntu/neo4j-databases* location. The instance should have created the *databases* and *transactions* folders. The *databases* folder should look as follows:
...
----
sudo /usr/bin/neo4j-admin load --database=movies --from=/home/ubuntu/files/movies.dump
----
[start=2]
. Change the ownership to neo4j:neo4j recursively for the sub-folder for _movies_ under the *databases* and *transactions* folders, for example:
----
sudo chown -R neo4j:neo4j /home/ubuntu/neo4j-databases/databases/movies
sudo chown -R neo4j:neo4j /home/ubuntu/neo4j-databases/transactions/movies
----
[start=3]
. Restart the Neo4j instance.
. Ensure that it starts with no errors.
