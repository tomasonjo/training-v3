...
[.note]
--
Using the wildcard operator `\*`, we project all the node labels in the database.
--
...
[.note]
--
Using the wildcard operator `\*`, we project all the relationship types in the database.
--
...
We project a graph consisting of nodes labeled _Person_ and their _seed_ property. 
...
We projected a graph that contains nodes labels _Person_ and _Organization_.
...
The _node query_ parameter is used to describe the nodes we want to project.
...
Using the _labels_ column in return allows us to filter node labels at algorithm execution time like with the Native projection.
