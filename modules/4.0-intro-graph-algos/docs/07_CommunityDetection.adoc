...
//as described in Hierarchical Modularity in Human Brain Functional Networks 21.
//?? need a reference here?

* Evaluating criminal networks and holes in the structure.

* Detecting cyber attacks. The algorithm was used in a study of fast community detection in large scale cyber networks for cyber security applications. Once these communities have been detected they can be used to detect cyber attacks.
//?? need a reference here?
...
. In NEuler:  Perform the Louvain Modularity algorithm on different *seasons* of Game of Thrones (GOT).
. In Neo4j Browser:
