...
. We leave the Store results box checked.
. We set Community Node Limit to 10 (or at least 6 or greater as we have 6 Person nodes in our database)
...
. We must ensure the Community Node Limit is at least 6, which is the number of Person nodes in the database.
...
. Next, we select the Weight property to be 'weight'.
...
. Let's refine the algorithm to use the the Natural relationship orientation.
. We run the algorithm to see that we still have 2 communities, but the louvain values are different because use factered in the direction of the relationship.
. Next, we select the Weight property to be 'weight'.
