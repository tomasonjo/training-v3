...
On the Twitter you can follow someone, but they don't necessarily need to follow you back.
This is an example of a directed graph, where the direction of the relationship matters.
While on Facebook, you can either be a friend or not, there is no one way relationship.
Facebook is an example of an unweighted graph.

A task workflow is usually a directed acyclic graph, where tasks follow one another in different steps.
If there are lot of relationships between the nodes in the graph we call that a dense graph, and is there are few relationships we call that a sparse graph.
...
Neo4j knows only directed relationships. We infer treat bidirected on undirected relationships the same.
If we project a two relationships one in each direction, we are effectively treating that relationship as undirected.

[.is-half.left]
--
image::bidirected-graph.png[Bidirected graph,width=500,align=center]
--
...
[.is-half.right]
--
We can have a number representing the strength of the relationship. This is called a weighted graph.
--
...
[.is-half.right]
--
It's important to see that sometimes a bigger number means a stronger relationships like the LIKES relationship in example.
Community detection and centrality algorithms treat higher values of weights as stronger relationships.
On the other hand for pathfinding algorithms, the smaller value of a weight is better, as we are mostly looking for shortest weighted paths.
--
...
image::singlegraph.png[SingleGraph,width=500,align=center]
...
[.is-half.right]
--
Centrality algorithms are used to find the most influental nodes in a network.
The most known algorithm is PageRank, implemented by Google.

Reveal which nodes are important based on graph topology to uncover the roles of individual nodes and their impact. They identify influential nodes based on their position in the network and include the famous PageRank algorithm. These algorithms are used to infer group dynamics such as credibility, rippling vulnerability and bridges between groups.
--
...
[.is-half.right]
--
Pathfinding algorithms usually are used to find the shortest path between nodes in a network.
The most common algorithm is the Dijkstra algorithm for finding the shortest weighted paths.

find the most efficient or shortest paths to traverse between nodes. The A* and Dijkstra’s algorithms are included in this category, which are used to evaluate routes for uses such as physical logistics and least-cost call or IP routing.
--
...
[.is-half.right]
--
Similarity algorithms are used to find similar nodes in a network based on graph topology or for example cosine similarity of an embedding.
The most commonly used is the Jaccard similarity index.
They are also used to project a monopartite graph from a bipartite, like we saw in the example above.

employ set comparisons to score how alike individual nodes are based on their neighbors or properties. This approach is used in applications such as personalized recommendations and developing categorical hierarchies.
--
...
consider the proximity of nodes, as well as structural elements, to predict unobserved or future relationships. Preferential Attachment is included in this class of algorithms that has many applications, from drug repurposing and estimating collaboration to criminal investigations.
--
