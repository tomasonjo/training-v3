...
== Exercise 6: Optimizing Property Access (Preparations)
...
== Exercise 6: Optimizing Property Access (Overview)
...
* *Exercise 6.3*: Putting it all together: Tune a query.
...
2. Optimize this query to use an index for property access.
...
[.thumb]
image::EX6.1.png[EX6.1,width=800]
...
[.thumb]
image::EX6.1A.png[EX6.1A,width=800]
...
2. Rewrite this query to reduce the the number of rows processed.
...
[.thumb]
image::EX6.2.png[EX6.2,width=800]
...
[.thumb]
image::EX6.2A.png[EX2.2A,width=800]

== Exercise 6.3: Putting it all together: Tune a query (Instructions)

1. Here is a complex Cypher query that finds all actors named _$actorName_ where this paramater is set to "Keanu Reeves".
It finds all movies that are three hops away. We don't want any duplicate actors so we test this with the `WHERE` clause.
We also fins the directors who directed the movie that is 3 hops away from _$actorName_. It then returns the data and sorts the data by movie title.
...
2. Rewrite this query to use an index to anchor the query.


3. The first _Movie_ label is necessary so that we only retrieve _Movie_ nodes, but later in the query, we use the _Movie_ label in two places.
This will slow things down.
Rewrite the query to not use the _Movie_ label later on in the query.

4. In the execution plan at the expansion, we see 39,373 rows returned and we know that we do not have this many movies in the graph.
Modify the query so that before after we do the initial query, we will be working with distinct movie nodes.


5. In the execution plan at the expansion, that there are still a lot of db hits around filtering out the movies that the actor did not act in.
...
6. We can still do more to optimize this query. Move the `LIMIT` and `ORDER BY` up in the query.


7. We still have two back-to-back matches near the end of the query. In most cases a movie only has one director, but we can still gain something by using pattern comprehension, rather than the `MATCH` clauses. Replace them with pattern comprehension in the `RETURN` clause.
...
[.thumb]
image::EX6.3.png[EX6.3,width=800]
...
[.thumb]
image::EX6.3A.png[EX6.3A,width=800]
...
[.thumb]
image::EX6.3B.png[EX6.3B,width=800]
...
[.thumb]
image::EX6.3C.png[EX6.3C,width=800]
...
[.thumb]
image::EX6.3D.png[EX6.3D,width=800]
...
[.thumb]
image::EX6.3E.png[EX6.3E,width=800]
...
[.thumb]
image::EX6.3F.png[EX6.3F,width=800]
...
ifdef::env-guide[]
pass:a[<a play-topic='{guides}/07.html'>Continue to Exercise 07</a>]
endif::[]
