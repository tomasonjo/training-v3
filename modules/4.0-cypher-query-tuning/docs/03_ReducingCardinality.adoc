...
This is much better. We see far fewer rows, no NodeByLabelScan, 96 db hits, and a smaller elapsed time.
...
Here is a query that uses an index:
...
Here we see fewer rows and fewer db hits, but the elapsed time is greater, which could be a problem.
...
It is slightly better and we definitely see fewer rows in the execution plan.
