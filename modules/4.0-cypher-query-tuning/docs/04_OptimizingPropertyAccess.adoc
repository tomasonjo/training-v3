...
Here is how the previous query could use a full-text schema index.
...
Here is another example where we want to only return _$numResults_ values.
...
With this query, the results are returned automatically in order:
