...
. Reduce gather-and-inspect behavior during a traversal.  In the *one node* example, if we want to find every address in the city of Axebridge, we would need to check the properties on *every* Person node, then discard most of them.  This is grossly inefficient.  In the *multiple node* version, this is a simple matter of locating the singular Axebridge node, and traversing to every Address node connected to it.  This model has no “wasted” hops.
