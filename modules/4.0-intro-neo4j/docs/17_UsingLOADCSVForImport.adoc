...
image::TransformMovieData1.png[TransformMovieData1,width=750,align=center]
...
Placing `USING PERIODIC COMMIT` enables the load, by default, to commit its transactions every 1000 rows which will enable the entire import of a large file to succeed.
However, there are certain types of Cypher constructs that will cause `USING PERIODIC COMMIT` to be ignored.
