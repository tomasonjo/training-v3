...
[none.answers]
- ◻️ [src-cypher]`MATCH (n) RETURN n.Customer.email`
- ◻️ [.fragment.check.ok]#✔️# [src-cypher]`MATCH (c:Customer) RETURN c.email`
- ◻️ [src-cypher]`MATCH (Customer) RETURN email`
- ◻️ [src-cypher]`MATCH (c) RETURN Customer.email`
...
- ◻️ [.fragment.check.ok]#✔️# [.answer]#When you want to use the node or relationship to return a result.#
